...
public class BendingProcess extends ProcessDiagram {
    ...
    public BendingProcess(Controller controller, Gui gui, XFrame xFrame) {
            ...
            for(int i = 1; i <= topShapeID; i++ ){
                for(int j = 0; j < m_xShapes.getCount(); j++ ){
                    ...
                    if (currShapeName.contains("RectangleShape")) {
                        ...
                        if(currShapeID == i)
                            addItem(new BendingProcessItem(this, currShapeID, xCurrShape, getPairOfMainShape(currShapeID)));
                    }
                }
            }
        } catch (Exception ex) {
    ...
    public ProcessDiagramItem createItem(int shapeID, String str) {
        ...
        setArrowShapeProps(xPolygonShape);
        
        BendingProcessItem item = new BendingProcessItem(this, shapeID, xRectangleShape, xPolygonShape);
        addItem(item);
        item.setText(str);
        ...
        if(isColorSchemeMode() || isColorThemeGradientMode()){
            if(isColorSchemeMode()){
                ...
                if(shapeID > 0){
                    ...
                    int numOfItems = getTopShapeID();
                    int numOfRows = 1;
                    if(numOfItems > 2){
                        do{
                            numOfRows++;
                        }while(numOfRows * numOfRows < numOfItems);
                        if((shapeID - 1) % (numOfRows * 2) < numOfRows)
                            setGradient(xShape, (short)900);
                        else
                            setGradient(xShape, (short)2700);
                        if(shapeID % numOfRows == 0 && getShapeName(xShape).contains("PolyPolygonShape"))
                            setGradient(xShape);
                    } else {
                        setGradient(xShape);
                    }
    ...
    public void refreshDiagram() {
        ...
        if(numOfItems > 0){
            ...
            for(ProcessDiagramItem item : items){
                ((BendingProcessItem)item).setPosition(numOfItems, controlShapeSize, controlShapePoint);
                if(isColorSchemeMode()){
    ...
    public int getSelectedShapeID(){
        ...
        if(shapeName.contains("BendingProcess") && (shapeName.contains("PolyPolygonShape") || shapeName.contains("RectangleShape"))){
    ...
    public void showPropertyDialog(){
        ...
        if(exec == 1){  
            getGui().setPropertiesOfBendingProcess();
    ...
    public void initProperties(XShape xControlShape, ArrayList<ProcessDiagramItem> items){
        ...
        try {
            if(getStyleProp() == BendingProcess.ROUNDED)
                setRoundedProp(Diagram.MEDIUM_ROUNDED);
            if(getStyleProp() == BendingProcess.WITHOUT_OUTLINE)
                setOutlineProp(false);
            if(getStyleProp() == BendingProcess.WITH_SHADOW){
                setShadowProp(true);
                setRoundedProp(Diagram.MEDIUM_ROUNDED);
            }
            if(getStyleProp() == BendingProcess.USER_DEFINE){
